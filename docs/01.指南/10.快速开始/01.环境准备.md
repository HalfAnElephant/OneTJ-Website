...
| 工具 | 最低版本 | 推荐版本 |
|------|---------|---------|
| **Flutter SDK** | — | `ohos_flutter_3.35.7`（鸿蒙版，`.fvmrc` 固定） |
...
将克隆的 SDK 放入 FVM 的版本文件夹中，并命名为 `ohos_flutter_3.35.7`：
...
当前 SDK 需要两个本地补丁：Windows 下的 Visual Studio 2026（18.x）兼容补丁，以及 `meta` 1.17 兼容补丁。详见「Flutter 开发 → 环境配置」。
...
```
[✓] Flutter (Channel unknown, 3.35.7, on macOS 14.5 23F79)
